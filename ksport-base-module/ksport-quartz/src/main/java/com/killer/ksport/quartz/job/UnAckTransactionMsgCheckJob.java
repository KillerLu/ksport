 ...
 * @description：未应答的事务消息检测
 * @modified By：
 * @version: version
 */
@DisallowConcurrentExecution//不允许并发执行
public class UnAckTransactionMsgCheckJob extends TransactionDealJob implements Job {
    ...
    protected List<TransactionMessage> listDueTimeMessage() {
        return transactionMsgApi.listUnAckMessage();
    }
    ...
    protected Object checkTransactionStatus(long keyId, String action) {
        switch (action) {
            case TransactionActionConstant.DELETE_GROUP_USER_BY_USERID:
                return groupServiceCaller.checkGroupUserByUserId(keyId);
        }
        return null;
    }
